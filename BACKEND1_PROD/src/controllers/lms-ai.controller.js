...
const DOMAIN_MAP = [
  {
    ...
    keywords: [
      ...
      'cloud', 'data scientist', 'machine learning', 'ml engineer', 'ai engineer', 'react', 'node',
      'python', 'java', 'typescript', 'angular', 'vue', 'docker', 'kubernetes', 'aws', 'azure',
...
const DOMAIN_DEPTH = {
  'IT / Software': {
    Junior: 'basic debugging, simple component logic, beginner data structures, git workflows',
    Mid: 'REST API design, state management, CI/CD pipelines, database normalization, code review scenarios',
...
1. Context Extraction:
   - Infer role (fallback: "Professional")
   - Infer experience level: Junior | Mid | Senior (fallback: "Mid")
   - Infer question count: 1–20 (fallback: 5)
...
4. Output schema (STRICT JSON — no markdown):
{
  ...
  "questions": [
    ...
    // Terminal print for visibility
    console.log(`\n[LMS-AI] GENERATED QUESTIONS FOR: ${parsedResponse.role}`);
    parsedResponse.questions.forEach((q, i) => console.log(`${i+1}. ${q.question}`));
    console.log('-----------------------------------\n');
