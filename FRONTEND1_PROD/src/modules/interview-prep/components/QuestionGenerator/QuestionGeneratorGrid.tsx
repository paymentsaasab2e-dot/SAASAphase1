      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        {items.map((item) => (
          <QuestionCard
            ...
            icon={ICONS[item.id as keyof typeof ICONS] ?? Cpu}
            onGenerate={() => onGenerate(item.id)}
          />
        ))}
      </div>

      <div className="mt-4 rounded-2xl border border-violet-100 bg-gradient-to-r from-violet-50/80 to-white p-1.5 shadow-sm">
